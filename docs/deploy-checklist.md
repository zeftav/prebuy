...
- [x] ✅ `structure-logbook` — Verify JWT **ON**. Logbook OCR import (Claude vision). (2026-06-27)
  ...
  - [x] ✅ **REDEPLOYED `structure-logbook` (JWT ON) for v0.47.0** (2026-08-03) — AD events now carry
        ...
        `ANTHROPIC_API_KEY`.
- [ ] 🔁 **REDEPLOY `report` (JWT OFF) for v0.48.0 + v0.49.0 + v0.50.0** — v0.49.0 rewrites the fn for
      **report revisions** (serve latest frozen snapshot + a self-verifying `publish` action), v0.48.0 adds
      `kind` on media (**video** on the report), v0.50.0 returns `inspection.compression` (**compression
      table**). One redeploy covers all three. Needs migration 031. Stays JWT OFF (publish self-verifies
      the Bearer). (2026-08-03)
