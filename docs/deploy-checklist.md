...
- [x] ✅ `structure-logbook` — Verify JWT **ON**. Logbook OCR import (Claude vision). (2026-06-27)
  ...
  - [x] ✅ **REDEPLOYED `structure-logbook` (JWT ON) for v0.47.0** (2026-08-03) — AD events now carry
        ...
        `ANTHROPIC_API_KEY`.
  - [ ] 🔁 **REDEPLOY `structure-logbook` (JWT ON) for v0.55.0** — accepts an optional `pdf_url` and reads
        it as a document input (the MM life-limited scan now takes a **PDF** upload, not just photos). No
